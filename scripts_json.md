...
The `scripts.json` file is the master registry for Aksharajanani. It defines the available templates (scripts) that users can choose from when starting a project. It acts as the central configuration hub, linking visual settings, font metrics, and external data files.
