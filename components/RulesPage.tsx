...
const AutoGeneratedBlockCard: React.FC<{
    ...
    isFirst: boolean;
    isLast: boolean;
}> = ({ featureTag, allCharsByUnicode, glyphDataMap, onMoveUp, onMoveDown, isFirst, isLast }) => {
    const { t } = useLocale();
    ...
    const count = useMemo(() => {
        // Lightweight count check (optional optimization, for now just calc full list length on expand or 0)
        // To show count badge without expanding, we'd need to run calculation. 
        // For performance, we can just show "Dynamic" or calculate if needed.
        // Let's calculate length lazily or just show "?" if collapsed.
        // Actually, getAutoGeneratedRules is fast enough for metadata checks.
            ...
            {isExpanded && (
                <div className="p-4 bg-white dark:bg-gray-900/50 rounded-b-lg animate-fade-in-up">
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                        {rules.map((rule, idx) => (
                            <div key={idx} className="flex items-center gap-2 text-xs p-2 rounded bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                                <span className="font-mono text-gray-500">sub</span>
                                <div className="flex gap-1">
                                    {rule.input.map((n, i) => (
                                        <span key={i} className="font-bold text-gray-700 dark:text-gray-300">{n}</span>
                                    ))}
                                </div>
                                <span className="font-mono text-gray-500">by</span>
                                <span className="font-bold text-indigo-600 dark:text-indigo-400">{rule.output}</span>
