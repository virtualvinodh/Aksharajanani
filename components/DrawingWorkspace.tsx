...
const DrawingWorkspace: React.FC<DrawingWorkspaceProps> = ({ characterSets, onSelectCharacter, onAddGlyph, onAddBlock, drawingProgress, isCompactView, isOverlayMode = false }) => {
    ...
    const visibleCharacterSets = useMemo(() => {
        ...
        // Return full sets if not filtered, allowing Virtuoso to handle rendering
    ...
    }, [characterSets, isFiltered]); // Removed showHidden dependency as CharacterGrid handles item filtering internally now
    ...
    // Calculate how many items in the filtered list are actually "Acceptable" (ready and not done)
    const autoGeneratedActionableCount = useMemo(() => {
        if (filterMode !== 'autoGenerated') return 0;
    ...
    // Check if the current selection consists ONLY of drawn glyphs (Transformable)
    const isSelectionTransformable = useMemo(() => {
        if (metricsSelection.size === 0) return false;
        for (const unicode of metricsSelection) {
            const char = allCharsByUnicode.get(unicode);
            if (!char) return false;
            // Virtual glyphs cannot be transformed geometrically in batch
            if (char.position || char.kern) return false;
        }
        return true;
    }, [metricsSelection, allCharsByUnicode]);
        ...
        filteredFlatList.forEach(char => {
            ...
            if (char.position) {
                ...
                if (baseChar && markChar && baseChar.unicode !== undefined && markChar.unicode !== undefined) {
                    ...
                    // Only process if NOT already positioned (Safe for mixed lists)
            ...
            if (char.kern) {
                 ...
                 if (leftChar && rightChar && leftChar.unicode !== undefined && rightChar.unicode !== undefined) {
                     ...
                     // Only process if NOT already kerned
    ...
    // Logic to determine if we show the "empty" state (depends on filtering)
    // If not filtered, we rely on Virtuoso to show content or empty groups
    // If filtered, we check flat list length
            ...
            <div className="flex-grow overflow-hidden">
                {showEmptyState ? (
                    ...
                    <CharacterGrid
                        ...
                        // Pass direct onAddGlyph to allow passing targetSet
                ...
                <DrawingBatchToolbar 
                    ...
                    onSelectVisible={() => { 
                        ...
                        if (isFiltered) {
                            ...
                            // Select from current active set only? Or all visible?
                            // "Select Visible" usually implies viewport, but for simplicity let's select current active group
