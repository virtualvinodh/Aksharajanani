...
const AddCharacterForm: React.FC<{
    ...
    return (
       <tr className="bg-gray-50 dark:bg-gray-800/50">
           <td className="p-1"><input type="text" placeholder={t('glyphNamePlaceholder')} value={newChar.name} onChange={e => setNewChar(c => ({...c, name: e.target.value}))} className="w-full p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"/></td>
            <td className="p-1"><input type="text" placeholder={t('unicodeHexOptional')} value={newChar.unicode} onChange={e => setNewChar(c => ({...c, unicode: e.target.value.toUpperCase().replace(/[^0-9A-F]/g, '')}))} className="w-24 p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600 font-mono"/></td>
            <td className="p-1"><select value={newChar.glyphClass} onChange={e => setNewChar(c => ({...c, glyphClass: e.target.value as any}))} className="w-full p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"><option value="base">base</option><option value="mark">mark</option><option value="ligature">ligature</option></select></td>
            <td className="p-1" colSpan={3}>
                 <div className="flex items-center gap-1">
                    <input type="number" placeholder={t('lsb')} value={newChar.lsb} onChange={e => setNewChar(c => ({...c, lsb: e.target.value}))} className="w-16 p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"/>
                    <input type="number" placeholder={t('rsb')} value={newChar.rsb} onChange={e => setNewChar(c => ({...c, rsb: e.target.value}))} className="w-16 p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"/>
                    <input type="number" placeholder={t('advWidth')} value={newChar.advWidth} onChange={e => setNewChar(c => ({...c, advWidth: e.target.value}))} className="w-20 p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"/>
                </div>
            </td>
            <td className="p-1" colSpan={2}>
                 <input type="text" placeholder="component1, component2" value={(newChar.isLinked ? newChar.link : newChar.composite).join(', ')} onChange={handleComponentChange} className="w-full p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"/>
                 ...
                 {newChar.composite.length > 0 && (
                    <div className="flex items-center gap-1 mt-1">
                        <input type="number" step="0.1" placeholder="scale (1.0)" value={newChar.compositeTransform[0]} onChange={e => setNewChar(c => ({...c, compositeTransform: [e.target.value, c.compositeTransform[1]]}))} className="w-24 p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600 text-xs" />
                        <input type="number" placeholder="y-offset (0)" value={newChar.compositeTransform[1]} onChange={e => setNewChar(c => ({...c, compositeTransform: [c.compositeTransform[0], e.target.value]}))} className="w-24 p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600 text-xs" />
                    </div>
                )}
            </td>
            <td className="p-1" colSpan={3}>
                <div className="flex items-center gap-1">
                    <input type="text" placeholder="Option Key" value={newChar.option} onChange={e => setNewChar(c => ({...c, option: e.target.value}))} className="w-24 p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"/>
                    <input type="text" placeholder="Description" value={newChar.desc} onChange={e => setNewChar(c => ({...c, desc: e.target.value}))} className="flex-grow p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"/>
                    <input type="text" placeholder="if='glyph_name'" title="Only include if 'glyph_name' is present" value={newChar.ifCond} onChange={e => setNewChar(c => ({...c, ifCond: e.target.value}))} className="w-28 p-1 border rounded bg-white dark:bg-gray-700 dark:border-gray-600"/>
                            ...
                            {set.characters.map((char, charIndex) => (
                                editingChar?.setIndex === setIndex && editingChar?.charIndex === charIndex ? (
                                    <tr key={charIndex} className="bg-indigo-50 dark:bg-indigo-900/20">
                                        <td className="p-1"><input type="text" value={editingChar.data.name} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, name: e.target.value}} : null)} className="w-full p-1 border rounded bg-white dark:bg-gray-700"/></td>
                                        <td className="p-1"><input type="text" value={editingChar.data.unicode} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, unicode: e.target.value}} : null)} className="w-24 p-1 border rounded bg-white dark:bg-gray-700"/></td>
                                        <td className="p-1"><select value={editingChar.data.glyphClass} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, glyphClass: e.target.value as any}} : null)} className="w-full p-1 border rounded bg-white dark:bg-gray-700"><option>base</option><option>mark</option><option>ligature</option></select></td>
                                        <td className="p-1" colSpan={3}><div className="flex items-center gap-1"><input type="number" value={editingChar.data.lsb} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, lsb: e.target.value}} : null)} className="w-16 p-1 border rounded bg-white dark:bg-gray-700"/><input type="number" value={editingChar.data.rsb} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, rsb: e.target.value}} : null)} className="w-16 p-1 border rounded bg-white dark:bg-gray-700"/><input type="number" value={editingChar.data.advWidth} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, advWidth: e.target.value}} : null)} className="w-20 p-1 border rounded bg-white dark:bg-gray-700"/></div></td>
                                        <td className="p-1" colSpan={2}><input type="text" placeholder="comp1, comp2" value={(editingChar.data.isLinked ? editingChar.data.link : editingChar.data.composite).join(', ')} onChange={handleComponentChange} className="w-full p-1 border rounded bg-white dark:bg-gray-700"/><label className="text-xs flex items-center gap-1 mt-1"><input type="checkbox" checked={editingChar.data.isLinked} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, isLinked: e.target.checked}} : null)} className="h-3 w-3" /> Link</label></td>
                                        <td className="p-1" colSpan={3}><div className="flex items-center gap-1"><input type="text" value={editingChar.data.option} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, option: e.target.value}} : null)} className="w-24 p-1 border rounded bg-white dark:bg-gray-700"/><input type="text" value={editingChar.data.desc} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, desc: e.target.value}} : null)} className="flex-grow p-1 border rounded bg-white dark:bg-gray-700"/><input type="text" value={editingChar.data.ifCond} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, ifCond: e.target.value}} : null)} className="w-28 p-1 border rounded bg-white dark:bg-gray-700"/><label className="text-xs flex items-center gap-1"><input type="checkbox" checked={editingChar.data.hidden} onChange={e => setEditingChar(s => s ? {...s, data: {...s.data, hidden: e.target.checked}} : null)} className="h-3 w-3" /> Hide</label></div></td>
