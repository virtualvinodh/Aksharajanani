...
const Chip: React.FC<{ label: string, color?: 'purple' | 'gray' | 'teal' | 'blue' }> = ({ label, color = 'gray' }) => {
    ...
    return (
        <span 
            className={`px-4 py-2 rounded text-base font-mono font-medium ${styles[effectiveColor]}`}
            style={{
    ...
    return (
        <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-6 animate-fade-in-up">
            <div className="flex justify-between mb-4">
                <h3 className="font-bold text-gray-900 dark:text-white">{rule ? t('editRule') : t('newPositioningRule')}</h3>
                <button onClick={onCancel}><CloseIcon className="w-5 h-5 text-gray-400" /></button>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                <div className="space-y-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase">{t('baseTargets')}</label>
                ...
                <div className="space-y-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase">{t('markTargets')}</label>
                    <div className="flex flex-wrap gap-2 mb-2 min-h-[32px]">
                         {mark.map(m => (
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
                <div>
                    <label className="text-xs font-semibold text-gray-500 uppercase block mb-1">{t('gposTag')}</label>
                ...
                <div>
                    <label className="text-xs font-semibold text-gray-500 uppercase block mb-1">{t('gsubTag')}</label>
                ...
                <div>
                    <label className="text-xs font-semibold text-gray-500 uppercase block mb-1">{t('movement')}</label>
    ...
    return (
        <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg border border-dashed border-gray-300 dark:border-gray-600 flex flex-col gap-3 animate-fade-in-up">
             <div className="flex justify-between items-center mb-2">
                 <h4 className="font-bold text-gray-800 dark:text-white text-sm">{isEditing ? t('editAnchor') : t('newAnchor')}</h4>
                 <button onClick={onCancel} className="text-gray-400 hover:text-gray-600"><CloseIcon className="w-4 h-4" /></button>
             </div>
             <div className="flex gap-4">
                 <div className="flex-1">
                     <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">{t('baseLabel')}</label>
                     <SmartGlyphInput value={base} onChange={setBase} characterSets={characterSets} groups={groups} placeholder="Base glyph..." />
                 </div>
                 <div className="flex-1">
                     <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">{t('markLabel')}</label>
                     <SmartGlyphInput value={mark} onChange={setMark} characterSets={characterSets} groups={groups} placeholder="Mark glyph..." />
                 </div>
             </div>
             <div className="flex gap-2 items-end">
                  <div className="flex-1">
                     <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">{t('basePoint')}</label>
                  ...
                  <div className="flex-1">
                     <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">{t('markPoint')}</label>
                  ...
                  <div className="w-20">
                      <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">{t('xOffset')}</label>
                  ...
                  <div className="w-20">
                      <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">{t('yOffset')}</label>
        ...
        <div className="space-y-8">
            <section>
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white">{t('featureRules')}</h3>
                        <p className="text-sm text-gray-500">{t('featureRulesDesc')}</p>
                    </div>
                    {!isAdding && !editingIndex && (
                        <button onClick={() => setIsAdding(true)} className="flex items-center gap-2 px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">
                            <AddIcon className="w-4 h-4" /> {t('add')} Rule
                        </button>
                    )}
                </div>
                ...
                <div className="space-y-3">
                    ...
                    {positioningRules.length === 0 && !isAdding && (
                        <p className="text-center text-gray-400 py-8 italic">{t('noRulesDefined')}</p>
                    )}
                </div>
            </section>
            ...
            <section>
                <div className="flex justify-between items-end mb-4">
                    <div>
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white">{t('anchorDefaults')}</h3>
                        <p className="text-sm text-gray-500">{t('anchorDefaultsDesc')}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        {!isAddingAnchor && !editingAnchor && (
                            <button onClick={() => setIsAddingAnchor(true)} className="flex items-center gap-2 px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">
                                <AddIcon className="w-4 h-4" /> {t('add')} Anchor
                            </button>
                        )}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    {visibleAttachments.length === 0 && !isAddingAnchor && (
                        <p className="col-span-full text-center text-gray-400 py-8 italic">{t('noAnchorDefaults')}</p>
                    )}
                </div>
            </section>
        </div>
