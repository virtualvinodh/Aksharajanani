...
export const getAutoGeneratedRules = (
    ...
    // 1. Position-based GSUB (Virtual Syllables)
    allCharsByUnicode.forEach(char => {
        // Condition: Is a composite, IS NOT GPOS (so must be GSUB), and matches feature tag
        if (char.position && !char.gpos && char.gsub === featureTag) {
    ...
    // 2. Composite-based GSUB (Explicit Ligatures)
    allCharsByUnicode.forEach(char => {
        if (char.composite && char.gsub === featureTag) {
            // Only handle simple pairs for now in auto-gen (standard ligature pattern)
            if (char.composite.length === 2) {
