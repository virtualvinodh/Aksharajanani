...
const getIndicFeatureForLigature = (components: Character[], scriptTag: string): string | null => {
    if (components.length < 2) return null;
    
    const secondChar = components[1];
    if (!secondChar || secondChar.unicode === undefined) return null;

    const indicData = indicScriptsData as any;
    const scriptViramas = indicData.virama[scriptTag] || [];
    const scriptMatras = indicData.matras[scriptTag] || { pres: [], abvs: [], blws: [], psts: [] };

    // 1. Check for Virama (Halant) -> 'akhn' (Akhand / Conjuncts)
    if (scriptViramas.includes(secondChar.unicode)) {
        return 'akhn';
    }

    // 2. Check for Vowel Signs (Matras) -> Position-based features
    if (scriptMatras.pres.includes(secondChar.unicode)) return 'pres';
    if (scriptMatras.abvs.includes(secondChar.unicode)) return 'abvs';
    if (scriptMatras.blws.includes(secondChar.unicode)) return 'blws';
    if (scriptMatras.psts.includes(secondChar.unicode)) return 'psts';
...
export const getAutoGeneratedRules = (
    ...
    allCharsByUnicode.forEach(char => {
        ...
        // Priority 3: Kern-based (Fused Pairs treated as Ligatures)
        // Must have kern, MUST NOT have GPOS (which implies standard kerning/dist, not substitution)
        if (char.kern && !char.gpos) {
            const [leftName, rightName] = char.kern;
            if (isNameDrawn(leftName) && isNameDrawn(rightName)) {
                rules.push({ input: [leftName, rightName], output: char.name });
                return; // Stop checking
            }
        }

        // Priority 4: Visual Construction (Composite OR Link)
...
export const generateFea = (
    ...
    clonedCharsByUnicode.forEach(char => {
        ...
        if (char.glyphClass === 'ligature' && char.liga && char.liga.length > 0 && !char.gsub) {
            if (isIndic) {
                // Resolve component characters
                const components = char.liga.map(name => allCharsByNameForFea.get(name)).filter((c): c is Character => !!c);
                
                // Try to infer specific Indic feature (akhn, abvs, etc.)
                const inferredFeature = getIndicFeatureForLigature(components, scriptTag);
                
                if (inferredFeature) {
                    char.gsub = inferredFeature;
                } else {
                    // Fallback for generic Indic ligatures (safer than 'liga')
                    // If it's just a base+base ligature without virama, it might be a discretionary ligature,
                    // but 'akhn' is often used for all mandatory conjuncts.
                    char.gsub = 'akhn'; 
                }
            } else {
                // Standard Latin/etc. behavior
                char.gsub = 'liga';
            }
        }
    });
        ...
        for (const child of effectiveChildren) {
            if (child.type === 'lookup') {
                ...
                if (inlineContent.trim() !== '') {
                 ...
                 const dynamicRules = getAutoGeneratedRules(clonedCharsByUnicode, glyphDataMap, featureTag);
