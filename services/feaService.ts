...
import { Character, KerningMap, MarkPositioningMap, PositioningRules, GlyphData, FontMetrics, Path, CharacterSet, MarkAttachmentRules, BoundingBox } from '../types';
import { getAccurateGlyphBBox, calculateDefaultMarkOffset } from './glyphRenderService';
import { DRAWING_CANVAS_SIZE } from '../constants';
import { isGlyphDrawn as isGlyphDrawnUtil, getGlyphExportNameByUnicode, isGlyphRenderable } from '../utils/glyphUtils';
import { expandMembers } from './groupExpansionService';
import { deepClone } from '../utils/cloneUtils';
import { getIndicCategory } from '../utils/indicCategoryUtils';
...
const INDIC_RA_UNICODES = new Set([
    0x0930, // Devanagari
    0x09B0, // Bengali
    ...
    0x0CB1, // Kannada Rra
    0x0D30, // Malayalam
    0x0D31, // Malayalam Rra
    0x0DBB, // Sinhala
]);

const isIndicRa = (unicode?: number): boolean => {
    return unicode !== undefined && INDIC_RA_UNICODES.has(unicode);
};

// Helper to determine the best GSUB feature for an Indic ligature
const getIndicFeatureForLigature = (components: Character[], scriptTag: string, allCharsByName: Map<string, Character>): string => {
    if (components.length < 2) return 'akhn';
    
    const seq = components.map(c => getIndicCategory(c.unicode!));
    
    // Check if the last component is a vowel sign
    const lastSeq = seq[seq.length - 1];
    const isLastVowel = lastSeq && lastSeq.syllabic === 'Vowel_Dependent';
    
    if (isLastVowel && components.length > 2) {
        // We have a sequence like C + Virama + C + ... + Vowel
        // Check if the base sequence (without the vowel) exists as a ligature
        const baseSequenceNames = components.slice(0, -1).map(c => c.name);
        
        // Check if any character has a liga array matching baseSequenceNames
        let baseAkhandExists = false;
        for (const char of allCharsByName.values()) {
            if (char.liga && char.liga.length === baseSequenceNames.length) {
                const matches = char.liga.every((val, index) => val === baseSequenceNames[index]);
                if (matches) {
                    baseAkhandExists = true;
                    break;
                }
            }
        }
        ...
        if (baseAkhandExists) {
            // The base Akhand exists, so this is a positional substitution on top of it
            if (lastSeq.positional === 'Right') return 'psts';
            if (lastSeq.positional === 'Bottom') return 'blws';
            if (lastSeq.positional === 'Top') return 'abvs';
            if (lastSeq.positional === 'Left') return 'pres';
            return 'psts'; // default fallback
        } else {
            // The base Akhand DOES NOT exist, so the whole thing must be substituted at once
            return 'akhn';
        }
    }

    // Check if it's a multi-consonant cluster: C + Virama + C [+ Virama + C]*
    let isConsonantCluster = true;
    for (let i = 0; i < seq.length; i++) {
        if (i % 2 === 0) {
            // Even indices should be Consonants
            if (seq[i].syllabic !== 'Consonant') {
                isConsonantCluster = false;
                break;
            }
        } else {
            // Odd indices should be Viramas
            if (seq[i].syllabic !== 'Virama') {
                isConsonantCluster = false;
                break;
            }
        }
    ...
    // If it ends on a Consonant (odd length) and follows the pattern, it's an Akhand
    if (isConsonantCluster && seq.length % 2 !== 0 && seq.length > 2) {
        return 'akhn';
    }
    ...
    if (components.length === 2) {
       ...
       // }
        ...
        if (seq[0].syllabic === 'Virama' && seq[1].syllabic === 'Consonant') {
            return 'pstf';
        }
        
        // Rule 4: Consonant + Vowel Sign (Positional Substitutions)
        if (seq[0].syllabic === 'Consonant' && seq[1].syllabic === 'Vowel_Dependent') {
            if (seq[1].positional === 'Right') return 'psts';
            if (seq[1].positional === 'Bottom') return 'blws';
            if (seq[1].positional === 'Top') return 'abvs';
            if (seq[1].positional === 'Left') return 'pres';
        }
    }
    
    // Rule 5: The Catch-All (akhn)
    return 'akhn';
};

export const inferGsubTag = (char: Character, allCharsByName: Map<string, Character>, isIndic: boolean, scriptTag: string): string | undefined => {
    if (char.gsub) return char.gsub;
    
    const hasLigatureComponents = char.liga && char.liga.length > 0;
    if (char.glyphClass !== 'virtual' && hasLigatureComponents) {
        if (isIndic) {
            const components = char.liga!.map(name => allCharsByName.get(name)).filter((c): c is Character => !!c);
            return getIndicFeatureForLigature(components, scriptTag, allCharsByName);
        } else {
            return 'liga';
        }
    }
...
export const getAutoGeneratedRules = (
    ...
    const isNameDrawn = (name: string): boolean => {
        ...
        return isGlyphRenderable(char, glyphDataMap, nameToCharMap);
    };

    allCharsByUnicode.forEach(char => {
        // Infer the GSUB tag if it's not explicitly set
        ...
        if (inferredTag !== featureTag) return;

        // Ensure the output character itself is drawn.
        // If not, the substitution would result in an invisible glyph.
        if (char.unicode !== undefined && char.unicode !== 8205 && char.unicode !== 8204) {
             if (!isGlyphRenderable(char, glyphDataMap, nameToCharMap)) return;
        }

        // Only generate GSUB rules if the 'liga' property is explicitly defined and the glyph is not virtual
        if (char.glyphClass !== 'virtual' && char.liga && char.liga.length > 0) {
...
export const generateFea = (
    ...
    markPositioningMap: MarkPositioningMap, // The manually set positions
    allCharsByUnicode: Map<number, Character>,
    fontName: string,
    ...
    // --- PRE-PROCESSING: GPOS & GSUB INFERENCE ---
    // Deep clone to prevent mutating global state (critical for UI previews)
    const clonedCharsByUnicode = deepClone(allCharsByUnicode);
    const allCharsByNameForFea = new Map<string, Character>();
    clonedCharsByUnicode.forEach(char => allCharsByNameForFea.set(char.name, char));
    
    // Determine Script
    const scriptTag = Object.keys(fontRules).find(key => key !== 'groups' && key !== 'lookups') || 'dflt';
    const indicScripts = new Set(indicScriptsData.indicScripts);
    const isIndic = indicScripts.has(scriptTag);
    ...
    clonedCharsByUnicode.forEach(char => {
        if (char.position && !char.gpos && char.glyphClass === 'virtual') {
            ...
            if (baseChar && markChar) {
                if (baseChar.glyphClass === 'mark' && markChar.glyphClass === 'mark') {
                    char.gpos = 'mkmk'; // Mark-to-Mark
                } else if (baseChar.glyphClass !== 'mark' && markChar.glyphClass === 'mark') {
                    char.gpos = 'mark'; // Base-to-Mark
                }
            }
        }
        
        // 1.5. GSUB Inference: Automatically assign feature tags for ligatures
        char.gsub = inferGsubTag(char, allCharsByNameForFea, isIndic, scriptTag);
    });

    // 2. Calculate Default GPOS Offsets
    const finalMarkPositioningMap = new Map(markPositioningMap.entries());
    const groups = fontRules?.groups || {};

    // Pass 1: From positioning.json rules
    if (positioningRules) {
        for (const rule of positioningRules) {
            ...
            for (const baseName of allBases) {
                for (const markName of allPossibleMarks) {
                    ...
                    const baseGlyphData = glyphDataMap.get(baseChar.unicode);
                    const markGlyphData = glyphDataMap.get(markChar.unicode);
                    
                    if (isGlyphDrawnUtil(baseGlyphData) && isGlyphDrawnUtil(markGlyphData)) {
                        const baseBbox = getAccurateGlyphBBox(baseGlyphData!, strokeThickness);
                        const markBbox = getAccurateGlyphBBox(markGlyphData!, strokeThickness);
    ...
    // Pass 2: From characters.json (now enriched with inferred gpos tags)
    clonedCharsByUnicode.forEach(char => {
    ...
    // --- FEATURE GENERATION ---
    // scriptTag is already determined at the start of the function for inference
    if (!scriptTag) {
    ...
    const nameToGlyphName = (name: string): string | null => {
        const char = allCharsByNameForFea.get(name);
        return char ? getGlyphName(char) : null;
    };
    
    // Helper function to check if a glyph has been drawn
    const isGlyphDrawnMemo = new Map<string, boolean>();
    const isGlyphDrawn = (char: Character | undefined, visited = new Set<string>()): boolean => {
        if (!char) return false;
        
        // Virtual characters are never "drawn" for the purpose of FEA generation
        // as they are not exported as physical glyphs.
        if (char.glyphClass === 'virtual') return false;
        
        if (isGlyphDrawnMemo.has(char.name)) return isGlyphDrawnMemo.get(char.name)!;
        ...
        isGlyphDrawnMemo.set(char.name, result);
        return result;
    };

    const isNameDrawn = (name: string): boolean => {
        return isGlyphDrawn(allCharsByNameForFea.get(name));
    };
    ...
    const isGroupNonEmpty = (groupName: string): boolean => {
        ...
        const expandedNames = expandMembers(memberNames, groups, characterSets);
        // Check if AT LEAST ONE member of the group is drawn.
        return expandedNames.some(isNameDrawn);
    };
    ...
    if (groups) {
        ...
        for (const groupName in groups) {
            if (Object.prototype.hasOwnProperty.call(groups, groupName)) {
                const memberNames = groups[groupName] as string[];
                const expandedNames = expandMembers(memberNames, groups, characterSets);
                const drawnMembers = expandedNames
                    .map(name => allCharsByNameForFea.get(name))
                    .filter(char => char && isGlyphDrawn(char) && char.glyphClass !== 'virtual')
    ...
    if (characterSets && characterSets.length > 0) {
        ...
        characterSets.forEach(set => {
            const drawnMembers = set.characters
                .filter(char => isGlyphDrawn(char) && char.glyphClass !== 'virtual')
                .map(char => getGlyphName(char))
                .filter((name): name is string => name !== null);
    ...
    clonedCharsByUnicode.forEach(char => {
        if (isGlyphDrawn(char) && char.glyphClass !== 'virtual') {
        ...
        for (const child of effectiveChildren) {
            if (child.type === 'lookup') {
                ...
                if (inlineContent.trim() !== '') {
                 ...
                 const dynamicRules = getAutoGeneratedRules(clonedCharsByUnicode, glyphDataMap, featureTag, scriptTag);
    ...
    // Priority 2: Populate with fallbacks from characters.json (using the cloned, enriched map).
    clonedCharsByUnicode.forEach(char => {
        if (char.position && char.gpos) {
            const [baseName, markName] = char.position;
    ...
    if (pairToGposTag.size > 0) {
        ...
        finalMarkPositioningMap.forEach((offset, key) => {
            const [baseUnicode, markUnicode] = key.split('-').map(Number);
            const baseChar = clonedCharsByUnicode.get(baseUnicode);
            const markChar = clonedCharsByUnicode.get(markUnicode);

            if (!baseChar || !markChar || !isGlyphDrawn(baseChar) || !isGlyphDrawn(markChar) || baseChar.glyphClass === 'virtual' || markChar.glyphClass === 'virtual') return;
            ...
            const baseBbox = getAccurateGlyphBBox(glyphDataMap.get(baseUnicode)!, strokeThickness);
            const markBbox = getAccurateGlyphBBox(glyphDataMap.get(markUnicode)!, strokeThickness);
    ...
    kerningMap.forEach((value, key) => {
        ...
        const leftChar = clonedCharsByUnicode.get(leftUnicode);
        const rightChar = clonedCharsByUnicode.get(rightUnicode);

        if (leftChar && rightChar && isGlyphDrawn(leftChar) && isGlyphDrawn(rightChar) && leftChar.glyphClass !== 'virtual' && rightChar.glyphClass !== 'virtual' && value !== 0) {
